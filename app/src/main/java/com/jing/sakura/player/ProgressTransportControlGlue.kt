...
class ProgressTransportControlGlue<T : PlayerAdapter>(
    ...
    private fun scheduleOverlayAutoHide(delayMillis: Long = 2600L) {
        overlayAutoHideJob?.cancel()
        if (!host.isControlsOverlayVisible) {
            return
        }
        overlayAutoHideJob = lifeCycleScope.launch {
            delay(delayMillis)
            if (host.isControlsOverlayVisible) {
    ...
    fun primeOverlayAutoHide(delayMillis: Long = 2200L) {
        scheduleOverlayAutoHide(delayMillis)
    }
    ...
    override fun onKey(v: View?, keyCode: Int, keyEvent: KeyEvent): Boolean {
        ...
        if (keyEvent.keyCode == KeyEvent.KEYCODE_BACK) {
            ...
            backPressed = true
            Toast.makeText(context, context.getString(R.string.player_exit_confirm), Toast.LENGTH_SHORT).show()
        ...
        if (host.isControlsOverlayVisible && keyEvent.action == KeyEvent.ACTION_DOWN) {
            scheduleOverlayAutoHide()
        }
        if (!host.isControlsOverlayVisible && keyEvent.action == KeyEvent.ACTION_UP) {
            when (keyCode) {
                KeyEvent.KEYCODE_DPAD_LEFT -> {
                    playerAdapter.seekTo((playerAdapter.currentPosition - 10_000L).coerceAtLeast(0L))
                    return true
                }

                KeyEvent.KEYCODE_DPAD_RIGHT -> {
                    val duration = playerAdapter.duration
                    val next = playerAdapter.currentPosition + 10_000L
                    playerAdapter.seekTo(
                        if (duration > 0) next.coerceAtMost(duration) else next
                    )
                    return true
                }
